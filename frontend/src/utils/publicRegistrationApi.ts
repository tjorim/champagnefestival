...
export async function requestRegistrationLookup(
  ...
  try {
    ...
    if (error instanceof RegistrationLookupError) {
      throw error;
    }
    throw new RegistrationLookupError("request_failed", m.my_reservations_error());
  }
}
...
export async function submitRegistration(
  ...
  if (!selectedEvent) {
    throw new RegistrationSubmitError(m.reservation_error());
  }
  ...
  try {
    ...
    if (error instanceof RegistrationSubmitError) {
      throw error;
    }
    throw new RegistrationSubmitError(m.reservation_error());
  }
