...
export function apiToRegistration(d: Record<string, unknown>): Registration {
  ...
  const rawOrders = Array.isArray(d.pre_orders)
    ? (d.pre_orders as unknown[])
        .filter((elem): elem is Record<string, unknown> =>
          typeof elem === 'object' && elem !== null && 'product_id' in elem
        )
    : [];
