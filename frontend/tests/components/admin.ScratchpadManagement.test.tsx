...
describe("ScratchpadManagement", () => {
  it("loads existing content, saves an edit, and shows a confirmation", async () => {
    ...
    server.use(
      ...
      http.put("/api/scratchpad", async ({ request }) => {
    ...
    render(<ScratchpadManagement authHeaders={() => ({ Authorization: "Bearer test" })} />, {
  ...
  it("shows an error alert when saving fails", async () => {
    server.use(
      ...
      http.put("/api/scratchpad", () => HttpResponse.json({ detail: "boom" }, { status: 500 })),
    );
