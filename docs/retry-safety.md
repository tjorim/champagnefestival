...
| Updates (`PUT`), including application settings, venue details/coordinates, and FAQ reorder | Browser and MCP admin clients | **Not retry safe.** They currently have no version precondition; clients must read and reconcile after an ambiguous result. Optimistic concurrency is the preferred strategy if automatic retries are added. |
