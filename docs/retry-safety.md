...
| Updates (`PUT`), including registration order/delivery updates, application settings, venue details/coordinates, and FAQ reorder | Browser, volunteer, and MCP admin clients | **Not retry safe.** They currently have no version precondition; clients must read and reconcile after an ambiguous result. Registration order updates re-resolve authoritative product data and preserve delivery counts, while volunteer delivery updates change only delivery counts, but concurrent stale writes can still overwrite newer state. Optimistic concurrency is the preferred strategy if automatic retries are added. |
