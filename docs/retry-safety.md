...
| Auto-claim by verified OIDC email (every `/api/me/*` request, `app.visitor_session.get_current_user`) | Signed-in browser | **Convergent, safe to blindly retry.** Every booking always collects an email regardless of sign-in state, so an unowned registration under an OIDC account's own `email_verified: true` claim is almost always the same person, not a separate identity to prove — trusted automatically instead of requiring the manual proof-token flow above, which stays only for a booking under a genuinely different email. Reuses the same existing-owner-protected `claim_unowned_registrations_for_email` the manual claim uses: a repeated call with the same email links nothing new once already claimed. No dedup needed. |
