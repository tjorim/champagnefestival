...
- [#802 — bourse seating model](https://github.com/tjorim/champagnefestival/issues/802): [owner-agreed booking/product guidance](decisions/802-booking-products-and-seating.md) now covers event-owned plans, manual/partial allocation with guest counts per table for split festival bookings, optional stock for all products, booked prices, nested included products, and merging accessibility text into notes. Implementation is split into product inventory/packages and physical seating. The first increment adds stock reservations, nested inclusions, booked-price snapshots, explicit package/price previews, payment/refund amounts and consolidated notes. Capsule-exchange companion rules remain unresolved. Existing plans are test data and need no historical event-matching workflow. Reconcile capacity/cancellation defects already addressed by #926/#927 and review Android/REST/MCP compatibility. The second increment adds event-owned plans across multiple rooms, split/shared/exclusive allocations with actual guest counts, partial assignment, cancellation release and atomic allocation updates through REST/MCP. The admin detail editor supports allocation editing; the full booking/payment/quantity editor and its combined table-release selection remain outstanding. Capsule-exchange companion policy is still undecided; #802 stays open.
