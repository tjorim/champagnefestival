...
| --- | --- | --- | --- | --- |
| [#1006](https://github.com/tjorim/champagnefestival/issues/1006) | Completed | 2026-09-11 | PR #1037; [decision](decisions/1006-volunteer-identity-self-service.md) | Volunteer-submitted-NISS self-claim links an OIDC subject to a `Person` (`Person.oidc_subject`, admin override for hand-linking/unlinking); self-service `GET`/`POST /api/me/volunteer/*` view own NISS/eID and request an eID correction as an admin-reviewed `ContactMessage`, mirroring the existing booking change-request pattern rather than writing directly. Stays behind `require_volunteer`; no new auth scope. |
