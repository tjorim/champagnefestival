"""Club member CRUD endpoints (admin-only).

Club members are stored in the people table as a subset with role='club-member'.
"""
...
def _register_routes(api: APIRouter) -> None:
    @api.post("", response_model=PersonOut, status_code=status.HTTP_201_CREATED)
    ...
    @api.get("", response_model=list[PersonOut])
    ...
    @api.get("/{person_id}", response_model=PersonOut)
    ...
    @api.put("/{person_id}", response_model=PersonOut)
    ...
    @api.delete("/{person_id}", status_code=status.HTTP_204_NO_CONTENT)
