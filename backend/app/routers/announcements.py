...
@router.post(
    "", response_model=AnnouncementOut, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_admin)]
)
async def create(
    body: AnnouncementCreate, request: Request, db: AsyncSession = Depends(get_db), actor: str = Depends(get_actor_id)
) -> dict:
    return await _call(
        service.create(db, actor=actor, body=body, request_id=getattr(request.state, "request_id", None))
    )
...
async def update(
    ...
    return await _call(
        service.update(
            db, actor=actor, item_id=item_id, body=body, request_id=getattr(request.state, "request_id", None)
        )
    )
...
async def reorder(
    body: AnnouncementReorder, request: Request, db: AsyncSession = Depends(get_db), actor: str = Depends(get_actor_id)
) -> list[dict]:
    return await _call(
        service.reorder(
            db, actor=actor, ordered_ids=body.ordered_ids, request_id=getattr(request.state, "request_id", None)
        )
    )
...
async def delete(
    item_id: str, request: Request, db: AsyncSession = Depends(get_db), actor: str = Depends(get_actor_id)
) -> None:
