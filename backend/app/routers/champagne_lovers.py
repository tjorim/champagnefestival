"""Champagne lovers CRUD endpoints (admin-only).

Champagne lovers are stored in the people table as a subset with
role='club-member'.
"""
...
def _ensure_club_member_role(person: Person) -> None:
...
async def create_champagne_lover(
    body: PersonCreate,
    ...
    _ensure_club_member_role(person)
...
async def list_champagne_lovers(
    db: AsyncSession = Depends(get_db),
...
async def get_champagne_lover(person_id: str, db: AsyncSession = Depends(get_db)) -> dict:
    person = await _get_or_404(db, person_id)
    return person_to_dict(person)


@router.put("/{person_id}", response_model=PersonOut)
async def update_champagne_lover(
    person_id: str,
    body: PersonUpdate,
    db: AsyncSession = Depends(get_db),
) -> dict:
...
async def delete_champagne_lover(person_id: str, db: AsyncSession = Depends(get_db)) -> None:
    person = await _get_or_404(db, person_id)
...
async def _get_or_404(db: AsyncSession, person_id: str) -> Person:
    ...
    if person is None or "club-member" not in person.get_roles():
        raise HTTPException(status_code=404, detail="Champagne lover not found.")
    return person
