...
async def test_claim_endpoint_still_requires_fresh_token_for_a_visitor_session(client, db_session, monkeypatch):
    """#953 decision 1: claim_my_registrations' shape is unchanged — a visitor
    session authenticates the caller but still requires a fresh one-shot
    lookup token to claim a *different* email's registrations.
    """
    from app.models import ReservationAccessToken

    captured = _capture_sent_token(monkeypatch)
    await client.post("/api/visitor-sessions/request", json={"email": "claimant@example.com"})
    token = captured[-1]
    await client.post("/api/visitor-sessions/redeem", json={"token": token})

    reg = await _post_registration(client, email="separate-owner@example.com")
    registration_id = reg.json()["id"]

    without_token = await client.post("/api/me/registrations/claim", json={"token": "x" * 25})
    assert without_token.status_code == 401

    lookup_token = "separate-claim-token-with-length"
    now = datetime.now(UTC)
    db_session.add(
        ReservationAccessToken(
            id="rat-separate-claim",
    ...
    claimed = await client.post("/api/me/registrations/claim", json={"token": lookup_token})
    assert claimed.status_code == 200
    assert [item["id"] for item in claimed.json()] == [registration_id]
    registration = await db_session.get(Registration, registration_id)
    user = await db_session.scalar(select(User).where(User.verified_email == "claimant@example.com"))
    await db_session.refresh(registration)
    assert registration.user_id == user.id
